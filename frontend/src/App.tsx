...
const NotFound: React.FC = () => (
  <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-50 px-6 text-center">
    ...
    <p className="mt-2 text-sm font-medium text-zinc-600">Página não encontrada.</p>
  </div>
);
    ...
    <AppProviders>
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/" element={<Navigate to={`/${DEV_DEFAULT_SLUG}`} replace />} />

          {/* Cadastro de novo negócio (rota dedicada, sem link no fluxo público) */}
          <Route path="/novo-negocio" element={<ProvisionPage />} />
